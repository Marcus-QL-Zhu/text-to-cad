...
from models.watch_kinematic.watch_kinematic.pattern4_independent_display_pattern import (
...
class Pattern4IndependentDisplayPatternTests(unittest.TestCase):
    def test_independent_display_pattern_card_declares_parallel_display_branches(self):
        ...
        self.assertEqual("pattern4_independent_hour_minute_no_seconds_v1", card["id"])
    ...
    def test_render_independent_display_pattern_markdown_names_parallel_contract(self):
        ...
        self.assertIn("# Pattern 4 - Independent Hour And Minute Display Without Seconds", markdown)
    ...
    def test_write_independent_display_pattern_card_writes_json_and_markdown(self):
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                [
                    output_dir / "pattern4_independent_hour_minute_no_seconds_v1.json",
                    output_dir / "pattern4_independent_hour_minute_no_seconds_v1.md",
                ],
                written_paths,
            )
    ...
    def test_independent_display_solver_outputs_independent_branches(self):
        ...
        self.assertEqual("pattern4_independent_hour_minute_no_seconds_v1", report["pattern_card_id"])
    ...
    def test_pattern4_solver_keeps_every_gear_tip_0_8mm_inside_case_wall(self):
    ...
    def test_write_independent_display_review_writes_gate1_html(self):
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(output_dir / "pattern4_independent_display_2d_review.html", review_path)
            html = review_path.read_text(encoding="utf-8")
            self.assertIn("Pattern 4 Independent Hour/Minute No-Seconds 2D Review", html)
